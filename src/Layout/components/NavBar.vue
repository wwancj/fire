<template>
  <div class="navbar-class">
    <div :class="homeClassMethod(home[0].path, 1, true)" @click="routerPath(home[0].path, home[0].children[0].path)">
      首页
    </div>
    <el-popover popper-class="popper-menu-class" placement="bottom-start" trigger="hover" v-for="(item, index) in routes" :key="index">
      <div slot="reference" :class="homeClassMethod(item.path, 1, item.roleId)" v-if="titles(item.meta)" disabled>
        {{ titles(item.meta) }}
      </div>
      <div solt="center" v-if="titles(item.meta)">
        <div
          :class="homeClassMethod(item1.path, 2, item.roleId)"
          v-for="(item1, index1) in item.children"
          :key="index1"
          @click="(item.roleId)?routerPath(item.path, item1.path):''"
        >
