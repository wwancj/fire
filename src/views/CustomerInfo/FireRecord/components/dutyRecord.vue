<template>
  <!--作者：万智宇-->
  <!--todo 值班记录-->
  <!--顶部搜索框-->
  <div class="role-class">
    <el-row :gutter="20">
      <el-col :span="24">
        <el-card shadow="never" class="card-10px">
          <el-select
            v-model="page.id"
            v-permission
            filterable
            clearable
            placeholder="请选择"
            size="mini"
            style="250px"
          >
            <el-option
              v-for="item in $unitOptions"
              :key="item.customerUnitId"
              :label="item.unitName"
              :value="item.customerUnitId"
            />
          </el-select>
          <el-button
            v-permission
            type="success"
            size="mini"
            icon="el-icon-search"
            @click="funSelectAllDutyRecordWithUnitName(page)"
          >搜索</el-button>
          <el-button type="warning" size="mini" icon="el-icon-refresh-left" @click="reset">重置</el-button>
          <el-button
            v-has="{action: 'add'}"
            type="primary"
            size="mini"
            icon="el-icon-plus"
            @click="userAdd"
          >新增</el-button>
        </el-card>
      </el-col>
      <!--主体显示信息的部分-->

      <el-col :span="24">
        <el-card shadow="never" class="el-table-custom-pgs table-auto">
          <div slot="header" class="clearfix">
            <span class="table-tree-top">值班记录</span>
            <el-popover placement="bottom-end" trigger="click">
              <div class="line-popver-pgs">
                <el-checkbox
                  v-model="checkAll"
                  :indeterminate="isIndeterminate"
                  @change="handleCheckAllChange"
                >全选</el-checkbox>
                <div style="margin: 15px 0;" />
                <el-checkbox-group v-model="checkedColumn" @change="handleCheckedCitiesChange">
                  <el-checkbox v-for="city in columnOptions" :key="city" :label="city">{{ city }}</el-checkbox>
                </el-checkbox-group>
              </div>
              <el-button slot="reference" size="mini" style="float: right;" icon="el-icon-s-grid" class="select-button"/>
            </el-popover>
            <el-tooltip class="item" effect="dark" content="刷新列表" placement="top">
              <el-button
                size="mini"
                style="float: right;"
                icon="el-icon-refresh"
                @click="funSelectAllDutyRecordWithUnitName(page)"
                class="select-button"
              />
            </el-tooltip>
          </div>
          <el-table
            :data="tableData"
            :row-class-name="tableRowClassName"
            style="width: 100%; margin-bottom: 16px;"
            stripe
            :header-cell-style="$store.state.style.headerStyle"
            :cell-style="$store.state.style.cellStyle"
          >
            <el-table-column type="index" :index="indexMethod" align="center" label="#" width="60" />
            <el-table-column v-if="isShow('单位')" prop="unitName" label="单位" min-width="75" />
            <el-table-column v-if="isShow('值班人员')" prop="name" label="值班人员" min-width="75" />
            <el-table-column v-if="isShow('日期')" prop="dutyDate" label="日期" min-width="70" />
            <el-table-column v-if="isShow('备注')" prop="remark" label="备注" min-width="70" />

            <el-table-column label="操作" fixed="right" width="160">
              <template slot-scope="scope">
                <el-button
                  v-has="{action: 'edit', effect: 'disabled'}"
                  type="primary"
                  size="mini"
                  icon="el-icon-edit"
                  title="修改"
                  @click="userEdit(scope.row)"
                />
                <el-popconfirm
                  icon="el-icon-info"
                  icon-color="red"
                  :title="`是否删除该数据`"
                  @confirm="userDel(scope.row)"
                >
                  <el-button
                    slot="reference"
                    v-has="{action: 'delete', effect: 'disabled'}"
                    type="danger"
                    size="mini"
                    icon="el-icon-delete"
                    title="删除"
                  />
                </el-popconfirm>
                <!-- <el-button type="warning" size="mini" icon="el-icon-setting" title="保留按钮" /> -->
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
            :current-page="page.pageNum"
            :page-sizes="[10, 20, 30, 40]"
            :page-size="page.pageSize"
            layout="total, sizes, prev, pager, next, jumper"
            :total="total"
            @size-change="handleSizeChange"
            @current-change="handleCurrentChange"
          />
          <!--分页效果-->
        </el-card>
      </el-col>
    </el-row>

    <!-- 添加 -->
    <el-dialog
      title="新增"
      :visible.sync="insertDialog"
      width="770px"
      @close="cancel('insertDutyRecord')"
      :modal-append-to-body="false"
      class="huise-class"
    >
      <el-form
        ref="insertDutyRecord"
        label-width="100px"
        class="from-format100"
        :inline="true"
        size="small"
        :model="insertDutyRecord"
        :rules="rules"
      >
        <el-form-item label="值班人员" prop="name">
          <el-input v-model="insertDutyRecord.name" />
        </el-form-item>
        <el-form-item label="日期" prop="dutyDate">
          <el-date-picker
            v-model="insertDutyRecord.dutyDate"
            type="date"
            placeholder="选择日期"
            validate-event
            value-format="yyyy-MM-dd"
          />
        </el-form-item>
        <el-form-item label="单位" prop="customerUnitId">
          <el-select v-model="insertDutyRecord.customerUnitId" filterable placeholder="请选择">
            <el-option
              v-for="item in $unitOptions"
              :key="item.customerUnitId"
              :label="item.unitName"
              :value="item.customerUnitId"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input v-model="insertDutyRecord.remark" />
        </el-form-item>
      </el-form>
      <!-- 弹窗底部按钮 -->
      <span slot="footer" class="dialog-footer">
        <el-button @click="cancel('insertDutyRecord')">取 消</el-button>
        <el-button type="primary" @click="addConfirm('insertDutyRecord')">确 定</el-button>
      </span>
    </el-dialog>

    <!-- 编辑 -->
    <el-dialog title="编辑" class="huise-class" :visible.sync="updateDialog" width="770px" @close="cancel('updates')" :modal-append-to-body="false">
      <el-form
        ref="updates"
        label-width="100px"
        class="from-format100"
        :inline="true"
        size="small"
        :rules="rules"
        :model="updates"
      >
        <el-form-item label="值班人员" prop="name">
          <el-input v-model="updates.name" />
        </el-form-item>
        <el-form-item label="上工日期" prop="dutyDate">
          <el-date-picker
            v-model="updates.dutyDate"
            type="date"
            placeholder="选择日期"
            validate-event
            value-format="yyyy-MM-dd"
          />
        </el-form-item>
        <el-form-item label="单位" prop="customerUnitId">
          <el-select v-model="updates.customerUnitId" filterable placeholder="请选择">
            <el-option
              v-for="item in $unitOptions"
              :key="item.customerUnitId"
              :label="item.unitName"
              :value="item.customerUnitId"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input v-model="updates.remark" />
        </el-form-item>
      </el-form>
      <!-- 弹窗底部按钮 -->
      <span slot="footer" class="dialog-footer">
        <el-button @click="cancel('updates')">取 消</el-button>
        <el-button type="primary" @click="editConfirm('updates')">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>

<script>
import { verifyUserPermission } from '@/utils/validate'
import {
  selectAllDutyRecordWithUnitName,
  insertDutyRecord,
  updateDutyRecordByPrimaryKey,
  deleteDutyRecordByPrimaryKey
} from '@/api/customer/dutyRecord'
export default {
  data() {
    return {
      rules: {
        name: [
          {
            required: true,
            message: '不能为空'
          },
          {
            max: 20,
            message: '最大20字'
          }
        ],
        dutyDate: [
          {
            required: true,
            message: '不能为空'
          }
        ],
        customerUnitId: [
          {
            required: true,
            message: '不能为空'
          }
        ],
        remark: [
          {
            max: 200,
            message: '最大200字'
          }
        ]
      },
      maxHeight: 600,
      unit: '所有单位',
      selectUserInfo: '',
      tableData: [],
      selectTime: '',

      columnOptions: [
        '值班记录编号',
        '日期',
        '联系人电话',
        '单位',
        '备注',
        '值班人员'
      ], // 源数据
      checkAll: true, // 全选
      checkedColumn: [], // checked组选项
      isIndeterminate: false, // 默认是否出现 半选
      columnlast: [], // 点击到最后一个的时候 保存一下 以便出现提示
      checkedColumnCopy: [], // 变成checked组选项 免得老是刷新抖动

      // David
      insertDutyRecord: {
        name: '',
        dutyDate: '',
        remark: '',
        customerUnitId: '',
        id: ''
      },
      insertDialog: false,
      updateDialog: false,
      updates: {},
      //	行号
      row_index: '',
      page: {
        pageNum: 1,
        pageSize: 10,
        id: null
      },
      total: 10
    }
  },
  mounted() {
    this.maxHeight =
      this.$store.state.clientHeights - 50 - 28 - 71 - 65 - 40 - 32 - 17 - 71
  },
  created() {
    this.columnOptions.forEach((element) => {
      this.checkedColumn.push(element)
      this.checkedColumnCopy.push(element)
    })
    this.funSelectAllDutyRecordWithUnitName()
  },
  methods: {
    reset() {
      this.page.id = ''
      this.funSelectAllDutyRecordWithUnitName()
    },
    indexMethod(index) {
      return index + 1 + (this.page.pageNum - 1) * this.page.pageSize
    },
    funSelectAllDutyRecordWithUnitName() {
      const temp = verifyUserPermission(this.$store.state.routersave.userInfo)
      this.page.id = temp || this.page.id
      selectAllDutyRecordWithUnitName(this.page).then((res) => {
        this.tableData = res.list
        this.total = res.total
      })
    },

    handleSizeChange(val) {
      this.page.pageSize = val
      this.funSelectAllOperator(this.page)
    },
    handleCurrentChange(val) {
      this.page.pageNum = val
      this.funSelectAllOperator(this.page)
    },

    // ----------------------- 可以主动隐藏列
    handleCheckAllChange(val) {
      if (val === true) {
        this.checkedColumn = val ? this.columnOptions : []
        this.isIndeterminate = false
      }
      this.checkAll = true
      this.columnCheckedCopy = this.columnOptions
    },
    handleCheckedCitiesChange(value) {
      if (value.length !== 0) {
        const checkedCount = value.length
        this.checkAll = checkedCount === this.columnOptions.length
        this.isIndeterminate =
          checkedCount > 0 && checkedCount < this.columnOptions.length
        if (value.length === 1) this.columnlast = value
        this.checkedColumnCopy = this.checkedColumn
      } else {
        this.checkedColumn = this.columnlast
        console.log(this.checkedColumn)
        this.$notify({
          title: '请至少选择一列',
          type: 'warning'
        })
      }
    },
    isShow(labelValue) {
      return this.checkedColumnCopy.indexOf(labelValue) !== -1
    },
    // -----------------------------可以主动隐藏列 结束
    // ----------------------------- 增加相关 开始

    // David
    userAdd() {
      // 添加用户
      this.insertDialog = true
    },
    addConfirm(formName) {
      // 添加用户 确认
      this.$refs[formName].validate((valid) => {
        if (valid) {
          insertDutyRecord(this.insertDutyRecord).then((res) => {
            if (res === 1) {
              this.$message({
                message: '插入成功',
                type: 'success'
              })
              this.clearInsertUser()
              this.funSelectAllDutyRecordWithUnitName()
              this.insertDialog = false
            }
          })
        } else {
          this.$message({
            message: '无法通过数据校验',
            type: 'error'
          })
        }
      })
    },
    clearInsertUser() {
      for (var key in this.insertDutyRecord) {
        this.insertDutyRecord[key] = ''
      }
    },
    cancel(formName) {
      this.$refs[formName].resetFields()
      this.insertDialog = false
      this.updateDialog = false
      this.clearInsertUser()
    },
    userSearch(row) {
      // 搜索功能
    },
    userDel(row) {
      // 用户删除
      deleteDutyRecordByPrimaryKey(row.id).then((res) => {
        if (res === 1) {
          this.$message({
            message: '删除成功',
            type: 'success'
          })
          this.page.pageNum = 1
          this.funSelectAllDutyRecordWithUnitName()
        }
      })
    },
    userEdit(row) {
      //	编辑用户
      this.row_index = row.index
      this.updates = JSON.parse(JSON.stringify(row))
      this.updateDialog = true
    },
    handleClose(done) {
      //  添加用户 确认关闭
      this.$confirm('确认关闭？')
        .then((_) => {
          done()
        })
        .catch((_) => {})
    },
    editConfirm(formName) {
      //	编辑用户 确认
      this.$refs[formName].validate((valid) => {
        if (valid) {
          updateDutyRecordByPrimaryKey(this.updates).then((res) => {
            if (res === 1) {
              this.$message({
                message: '编辑成功',
                type: 'success'
              })
              this.funSelectAllDutyRecordWithUnitName()
              this.updateDialog = false
            }
          })
        } else {
          this.$message({
            message: '编辑失败，请检查表单'
          })
        }
      })
    },
    tableRowClassName({ row, rowIndex }) {
      // 把每一行的索引放进row
      row.index = rowIndex
    }
  }
}
</script>
